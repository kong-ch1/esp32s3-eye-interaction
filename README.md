...
| 分支 | 对应阶段 | 说明 |
|---|---|---|
| **`week2`**（默认分支） | **第 2 周** | 双向指令通道 + 回执状态机（网页能反过来指挥设备再采一次） |
| `week1` | 第 1 周 | 单源采集 + 上行链路 + Web 实时展示 |
| `main` | 最新主线 | 含全部已完成周次，后续开发在此继续 |

**怎么切换**：仓库页面左上角的分支下拉框（当前显示 `week2`），选 `week1` 就回到第 1 周结束时的状态。

**第 2 周的完整改动范围**（相对第 1 周基线）：

```bash
# week1 分支 = 第 1 周结束时的状态（提交 88c3722）
git diff week1..week2 --stat
```
...
| 类别 | 内容 |
|---|---|
| 服务端 | 指令状态机 `commands.py`、`/api/command` 路由、在 `/api/data` 响应里捎带下发、readings 表加 `request_id`/`trigger`/`cmd_state` 三列 |
| 固件 | 读回响应体、cJSON 解析、抽出 `build_payload()` 供两种触发方式复用、`run_command()` 执行突发采集 |
| 网页 | 主动交互面板、四阶段时间线、表格「触发」列与指令数据高亮 |
| 测试工具 | 指令通道自测、真机验收、网页静态检查、**真实浏览器实测**（4 个，均在 `week1/tools/`） |
| 文档 | 第 2 周设计文档、第 2 周验收记录、部署说明（用电脑当服务器） |
| 运行时快照 | 编译/烧录/验收日志、实测数据库 |

> **为什么这里不写死提交数和改动行数**：每次更新这份文档本身就会改变它们
> （"更新文档" 也是一个提交），写死了会立刻失真。用上面那条
> `git diff week1..week2 --stat` 随时取准确值。
