...
| 9 | `docs` | 分支说明 |
| 10 | `test(tools)` | **真实浏览器实测**（Chrome 无头 + DevTools Protocol） |
| 11 | `feat(server)` | 启动时打印局域网地址 |
| 12 | `docs` | 服务部署说明（用电脑代替 VPS）+ 验收记录补浏览器实测 |
| 13 | `chore` | 工作日志同步与运行时数据快照 |
