...
| 方法   | 路径                      | 说明                                                           |
| ---- | ----------------------- | ------------------------------------------------------------ |
| POST | `/api/data`             | 板端上传一条记录，字段：`device_id`(必填)、`seq`、`ts`、`ax/ay/az`、`gx/gy/gz`、`temp_c`、`free_heap`、`min_free_heap`、`total_heap`、`rssi`、`request_id`、`trigger`、`cmd_state`。**响应体里会捎带待执行的下行指令** |
